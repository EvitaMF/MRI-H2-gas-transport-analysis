...
excel_path = r"C:\Users\evita\OneDrive - University of Bergen\MRI_H2\diffusion_study\2026 02 Diffusion\MRI Scan data\H2 N2 110 diff45 horisontal\H2 N2 sidelengs\H2_N2_horisontal.xlsx"

# Check that this path points to the correct side-by-side workbook before running.
DATASET_LABEL = "H$_2$–N$_2$ (side-by-side)"

SELECTED_SLICE = 11

# Velg hvilken kurve som skal fittes:
# "H2" = ROI1_left_H2_mean
# "N2" = ROI2_right_N2_mean
FIT_SIGNAL = "N2"

# Sett til None dersom du vil bruke alle datapunkter
# Sett til et tall i sekunder dersom du vil kutte, f.eks. 256695
TIME_CUTOFF = None
...
if FIT_SIGNAL not in signal_config:
    raise ValueError("FIT_SIGNAL must be either 'H2' or 'N2'.")
...
required_cols = ["Time_seconds", "Slice", mean_col, std_col]
...
# Sørg for at tiden starter på 0 for valgt slice
...
if len(t_fit_data) < 4:
    raise ValueError("Too few data points for fitting. Increase TIME_CUTOFF or check selected slice.")

print(f"\nDataset: {DATASET_LABEL}")
print(f"Selected slice: {SELECTED_SLICE}")
print(f"Fitted chamber: {fitted_chamber}")
print(f"Fitted signal column: {mean_col}")
print(f"Using {len(t_fit_data)} points, t <= {TIME_CUTOFF:.1f} s")
...
t_fit_curve = np.linspace(t_fit_data.min(), t_fit_data.max(), 500)
t_fit_curve_hours = t_fit_curve / 3600
S_fit_curve = exp_model(t_fit_curve, S_eq_fit, S0_fit, tau_fit)
...
# Shaded excluded region, if cutoff is before end of data
...
# Plot limits
plt.xlim(0, max(t_fit_data_hours.max() * 1.05, 0.1))
...
save_path = os.path.join(
    output_folder,
    f"diffusion_fit_{save_name}_slice{SELECTED_SLICE}.png"
)
